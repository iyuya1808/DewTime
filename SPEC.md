...
さらに本アプリは**ダイバシティ（多様性）への配慮**を設計の中核に置き、文字を読むことが困難なユーザー（未就学児、読字困難、発達障害のある方など）でも直感的に操作できることを目指します。設定画面を除く全画面からテキストラベルを排除し、アイコン・数字・水位のビジュアル・色・ハプティクスだけで完全に操作できる「テキストレス UI」を採用しています。
...
- 出発時刻設定画面（StartSheet）では、タンクを上下にスワイプするだけで出発までの時間（5〜60分）をダイレクトに設定できます。水位が高いほど長い時間を意味します。
- 数字とアイコンのみで操作が完結するため、文字が読めないユーザーでも直感的に使用可能です。
...
### 3.6 ダイバシティ配慮 UI 設計原則

DewTime は「文字を読まなくても使えるアプリ」を設計目標の一つとして掲げ、以下のユーザー層への配慮を実装しています。
...
| 除去した要素 | 代替表現 |
|---|---|
| 「出発まで」「遅刻中」ラベル | `figure.walk` / `exclamationmark.triangle.fill` アイコン |
| 「スタート」ボタン | `play.circle.fill`（80pt 大型アイコン）|
| 「いってきます！」ボタン | `figure.walk.departure` アイコン |
| 「キャンセル」ボタン | `xmark.circle` アイコン |
| 水位パーセント（%）表示 | 水のビジュアル高さ＋残り時間数字のみ |
| 図鑑フィルターテキスト | `checkmark.seal.fill`（解放済）/ `lock.fill`（未解放）/ `1.circle`〜`3.circle`（難易度） |
| タブバーラベル | アイコンのみ |
| 水槽画面の匹数テキスト | 魚アイコン＋数字バッジ |
| ヒントテキスト | 除去（初回のみ視覚的チュートリアルで補完） |

#### 保持する要素

- **カウントダウン数字（MM:SS 形式）** — 数字は言語・文化を問わず直感的に理解可能
- **時刻数字（7:30 など）** — 直感的理解が可能
- **プリセット数字（15 / 30 / 45 / 60）** — 時間量を示す最小限の数字
- **VoiceOver 対応** — 全インタラクティブ要素に `.accessibilityLabel()` を設定し、スクリーンリーダー利用者に対してはテキストで正確に読み上げ
- **設定画面（ProfileView）** — スケジュール・ルーティン編集など複雑な設定操作はテキストを維持

#### ジェスチャー優先インタラクション

- **スワイプで時間設定**: 出発時刻設定シート（StartSheet）にてタンクを上下にドラッグして出発時間を設定。時間量と水位が直接対応するため、数字が読めなくても「水が多い = 時間が多い」という身体感覚で理解できる。
- **色による状態表現**: タイマー状態を水の色で表現（十分な余裕: 青系 → 注意: 黄系 → 急いで: 橙系）。
- **ハプティクス**: 操作確認・フェーズ切替・出発完了をバイブレーションで通知し、画面を見なくても進行状況を把握可能。

### 3.7 チュートリアルとオンボーディング
...
### 3.8 アプリ内レビュー促進 (`ReviewRequestManager`)
...
└── タブ画面（TabView）
    ...
    │       │       タンクを上下スワイプして出発時間（5〜60分）を設定。
    │       │       右側に 15/30/45/60 分のプリセットをミニタンクのビジュアルで表示。
    │       │       魚選択は横スクロールのアイコン群。全操作をテキストなしで完結。
    │       ├── 1.3 タイマー稼働中（WaterTankView - 貯水タンクアニメーション）
    │       │       状態表示はアイコンのみ（遅刻中: ⚠ / 通常: 人物アイコン）。
    │       │       スタート: play.circle.fill 大型アイコン。
    │       │       出発: figure.walk.departure アイコン。
    │       ├── 1.4 出発確認（DepartureConfirmView）
    │       │       テキスト除去。成長段階アイコン群＋ビジュアルゲージで状態表現。
    │       ├── 1.5 注水トランジション（PourTransitionView）
    │       └── 1.6 出発完了・結果画面（DepartureResultView）
    │               テキスト除去。数字（経過時間・遅延）＋アイコンのみ。レビュー要求トリガー。
    │
